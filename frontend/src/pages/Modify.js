...
const Modify = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    var startTime = `${formData.fromHour.padStart(
      2,
      "0"
    )}:${formData.fromMin.padStart(2, "0")}`;
    var endTime = `${formData.toHour.padStart(
      2,
      "0"
    )}:${formData.toMin.padStart(2, "0")}`;
    ...
    try {
      const response = await fetch(
        `https://fall2024-comp307-group16.cs.mcgill.ca/apiapi/members/${id}/edit_booking`,
        {
          method: "POST",
          headers: {
      ...
      alert("Error in booking modification");
    }
